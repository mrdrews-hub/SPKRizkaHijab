...
class Rangking extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('template',"saw/index");
    }

    public function noData()
    {
        $this->template->load("template","saw/noData");
    }
    private function initialTableSAW($alternatif)
    {
        $nilai = $this->MNilai->getNilaiAlternatif();
        
        $dataInput = array();
        $no = 0;
        foreach ($alternatif as $item => $Itemalternatif) {
            foreach ($nilai as $index => $itemNilai) {
                if ($Itemalternatif->kdAlternatif == $itemNilai->kdAlternatif) {
